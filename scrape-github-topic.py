...
def get_top_repositories(doc):
    ...
    #class="d-flex flex-justify-between my-3"
    div_tags = doc.find_all('div', class_ = "d-flex flex-justify-between my-3")
    #print(div_tags)
    topic_repos = [parse_repository(tag) for tag in div_tags]
...
def parse_repository(article_tag):
    ...
    stars_tag = article_tag.find('span', id ='repo-stars-counter-star')
